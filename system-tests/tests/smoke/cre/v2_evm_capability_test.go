...
// ExecuteEVMLogTriggerUserErrorTest purposefully causes a user error in the log trigger workflow and checks that the error
// is found in the beholder logs with the correct visibility, origin and code. This ensures that user errors from remote and local
// log trigger workflows are correctly reported.
func ExecuteEVMLogTriggerUserErrorTest(t *testing.T, testEnv *ttypes.TestEnvironment) {
	...
	for chainID, bcOutput := range chainsToTest {
		...
		_, err := t_helpers.WaitForBaseMessageLabel(ctx, lggr, baseMessageCh, "err", expectedBeholderLog)
		if err != nil {
