...
var ErrUnableToDetermineRegistrationStatus = errors.New("terminated before before registration status could be determined")
...
// If the registration status is unable to be determined in the given maxWaitTime ErrUnableToDetermineRegistrationStatus is returned.
func (sr *SubscriberRegistration) AwaitRegistration(ctx context.Context, maxWaitTime time.Duration) error {
	...
	default:
		...
		select {
		case <-ctx.Done():
			sr.lggr.Warnw("context is done before await complete", "error", ctx.Err())
			return ctx.Err()
		case <-timer.C:
			return ErrUnableToDetermineRegistrationStatus
