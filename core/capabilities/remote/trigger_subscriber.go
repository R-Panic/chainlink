...
func (s *triggerSubscriber) RegisterTrigger(ctx context.Context, request commoncap.TriggerRegistrationRequest) (<-chan commoncap.TriggerResponse, error) {
	...
	registrationStatusUpdateTimeout := cfg.remoteConfig.RegistrationStatusUpdateTimeout
	if registrationStatusUpdateTimeout <= 0 {
		s.lggr.Infow("not waiting for registration status update as timeout is 0", "donId", cfg.capDonInfo.ID, "workflowID", request.Metadata.WorkflowID)
		return reg.GetTriggerResponseChannel(), nil
	}

	s.lggr.Infow("waiting for registration status update", "registrationStatusUpdateWaitTime", registrationStatusUpdateTimeout, "donId", cfg.capDonInfo.ID, "workflowID", request.Metadata.WorkflowID)
	...
	regErr := reg.AwaitRegistration(ctx, registrationStatusUpdateTimeout)

	if errors.Is(regErr, registration.ErrUnableToDetermineRegistrationStatus) {
		s.lggr.Warnw("no registration update received after wait", "registrationStatusUpdateWaitTime", registrationStatusUpdateTimeout, "donId", cfg.capDonInfo.ID, "workflowID", request.Metadata.WorkflowID)
	} else {
		s.lggr.Infow("registration status update received", "donId", cfg.capDonInfo.ID, "workflowID", request.Metadata.WorkflowID, "registration error", regErr)
	}
	...
	// due to subsequent registration refreshes.
