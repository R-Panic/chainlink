...
func Test_RemoteTriggerCapability_WaitForTriggerRegistration(t *testing.T) {
	...
	cfg := &commoncap.RemoteTriggerConfig{
		...
		MaxBatchSize:                    1, // no batching
		BatchCollectionPeriod:           time.Second,
		RegistrationStatusUpdateTimeout: 1 * time.Hour,
	}
...
func Test_RemoteTriggerCapability_WaitForTriggerRegistrationTimeout(t *testing.T) {
	...
	cfg := &commoncap.RemoteTriggerConfig{
		...
		MaxBatchSize:                    1, // no batching
		BatchCollectionPeriod:           time.Second,
		RegistrationStatusUpdateTimeout: 10 * time.Millisecond,
	}
	...
	triggerRegistrations := result.GetAll()
	for i, reg := range triggerRegistrations {
		require.ErrorIs(t, registration.ErrUnableToDetermineRegistrationStatus, reg.err, "expected error for trigger registration at index %d", i)
	}
}
...
func Test_RemoteTriggerCapability_TriggerUnregisters(t *testing.T) {
	...
	cfg := &commoncap.RemoteTriggerConfig{
		...
		MaxBatchSize:                    1, // no batching
		BatchCollectionPeriod:           time.Second,
		RegistrationStatusUpdateTimeout: 10 * time.Second,
	}
...
// Test_RemoteTriggerCapability_TriggerRegistrationErrorRecovery tests that when trigger registration fails
// due to underlying errors, the remote trigger capability will eventually recover and succeed if the underlying
// trigger registration succeeds later.
func Test_RemoteTriggerCapability_TriggerRegistrationErrorRecovery(t *testing.T) {
	ctx := t.Context()
	...
	f := uint8(4)
	cfg := &commoncap.RemoteTriggerConfig{
		RegistrationRefresh:             10 * time.Millisecond,
		RegistrationExpiry:              100 * time.Second,
		MinResponsesToAggregate:         uint32(f + 1),
		MessageExpiry:                   100 * time.Second,
		MaxBatchSize:                    1, // no batching
	...
	// Now clear the underlying trigger error and expect events to flow eventually after registration retry succeeds
	broadcaster.SetUnderlyingTriggerError(nil)
	payload := sendTriggerEvent(t, "Hello, ", broadcaster)
